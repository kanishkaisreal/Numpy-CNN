...
def cnn_convolution(image, filt, bias, stride=1):
    ...
    num_channels, image_size, _ = image.shape  # image dimensions

    out_dim = int((image_size - filter_size)/stride) + 1  # calculate output dimensions

    assert num_channels == num_channel_filter, "Dimensions of filter must match dimensions of input image"

    out = np.zeros((num_filter, out_dim, out_dim))
    # convolve the filter over every part of the image, adding the bias at each step.
    for curr_f in range(num_filter):
        curr_y = out_y = 0
        while curr_y + filter_size <= image_size:
            curr_x = out_x = 0
            while curr_x + filter_size <= image_size:
...
def cnn_predict(image, f1, f2, w3, w4, b1, b2, b3, b4, conv_s=1, pool_f=2, pool_s=2):
    '''
